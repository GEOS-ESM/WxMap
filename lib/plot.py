...
#       rgba_f  = self.refine_rgb(rgba, nsub)
    ...
    def plot_logos(self):
        ...
        for l in logos:
            ...
            self.options['_logos_'].append(logo)
...
#       dependency. If so, get the value of the dependency.
        ...
        result = self.config(apath + [value, stream], None)
        if result is not None: return result
        ...
        for name in listing:
            ...
            if not filelist:
                ...
                for dir in paths.split(':'):
                    files += self.get_files([os.path.join(dir,name)])
            ...
            for pathname in filelist:
                ...
                if os.path.isfile(pathname):
                    ...
                    files += self.get_files(glob.glob(os.path.join(pathname,'*')))
