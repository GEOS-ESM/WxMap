...
class Config(dict):
    ...
    def check_node(self, config: Dict) -> Dict:
        ...
        Returns:
            ...
            that corresponds to the current server, and any
            top-level "<key>_<node>" overrides applied to "<key>".
        """
        ...
        # Generic "<key>_<node>" override convention, e.g. a config with
        # both "shape_path" and "shape_path_discover" will use the latter
        # when running on discover. Lets settings like wxmap.rc carry
        # server-dependent defaults without any code changes.
        suffix = f'_{self.node}'
        for key in list(config.keys()):
            if key.endswith(suffix) and key != suffix:
                config[key[:-len(suffix)]] = config[key]
