...
class Service(GaNum,DataService):
    ...
    def __init__(self, config=None, window=False, verbose=False):
        ...
        grads_bin = (self.config or {}).get('grads_bin', 'grads')

        try:
            GaNum.__init__(self, Bin=grads_bin, Window=window, Echo=False, Verbose=verbose)
        except:
...
#------------------------------------------------------------------------------
    ...
    def check_cmd(self, cmd):
        ...
        if 'set mpvals' in cmd:
            ...
            if len(self.mpvals) == 4:
                cmd = 'set mpvals %f %f %f %f'%tuple(lons+self.mpvals[2:])
            else:
                cmd = 'set mpvals %f %f'%tuple(lons)
            return cmd
