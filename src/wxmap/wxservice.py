...
class WXService(object):
    ...
    def __init__(self, request=None,
        ...
        # wxmap.local.rc.example), then an explicit --rc file if requested.
        self.config.mount(_load_rc(default_file), '/')

        if os.path.isfile(local_file):
            self.config.mount(_load_rc(local_file), '/')

        if requested_file:
            self.config.mount(_load_rc(requested_file), '/')
...
#------------------------------------------------------------------------------
    ...
    def _rc_files(self, install_path, requested_file):
        """Compute the layered rc file paths: tracked defaults, an optional
        gitignored per-checkout override (wxmap.local.rc), and an explicit
        --rc file if one was requested and differs from the default."""

        default_file = os.path.join(install_path, 'utils', 'wxmap.rc')
        local_file   = os.path.join(install_path, 'utils', 'wxmap.local.rc')

        if requested_file == default_file:
            requested_file = None

        return default_file, local_file, requested_file

#------------------------------------------------------------------------------

    def _resolve_rc(self, path, app_path, resource_dict, install_path=None):

        kwargs = dict(resource_dict)
        if install_path is not None:
            kwargs['install_path'] = install_path

        res = self.config.read_resolve(path, **kwargs)
        return {k:v.replace('$app_path',app_path).replace('app_path',app_path)
                if isinstance(v,str) else v for k,v in res.items()}

    def _mount_resource_dict(self,request):
        app_path=str(self.request.get('app_path','app_path'))
        resource_dict = self.request.get('resource_dict',{})
        install_path = str(Path(__file__).parents[2])
...
class WXServiceLite(WXService):
    ...
    def __init__(self, request=None,
        ...
        install_path = str(Path(__file__).parents[2])
        default_file, local_file, requested_file = self._rc_files(
            install_path, self.request.get('rc', None))
        ...
        # optional gitignored per-checkout override (wxmap.local.rc, see
        # wxmap.local.rc.example), then an explicit --rc file if requested.
        resource = _load_rc(default_file)
        file = default_file

        if os.path.isfile(local_file):
            self.config.overlay(resource, _load_rc(local_file))
            file = local_file

        if requested_file:
            self.config.overlay(resource, _load_rc(requested_file))
            file = requested_file
